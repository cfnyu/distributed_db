...
class InputFourTestCase(unittest.TestCase):
    """ Test cases for Input File Four """
    ...
    def test_input_4(self):
        """ Test the four input file """

        parser = Parser(self.input_dir + "/input_4", self.logger)
        ...
        read_t1_x1 = parser.get_instruction()
        self.assertEquals(read_t1_x1.instruction_type, InstructionType.READ)
        self.assertEquals(read_t1_x1.transaction_identifier, "T1")
        self.assertEquals(read_t1_x1.variable_identifier, "x1")

        with std_out() as (out, err):
            self.transaction_manager.execute(read_t1_x1)

        output = out.getvalue().strip()
        self.assertEquals(output, "T1: Read x1 - value 10 at site 2")
        ...
        self.assertEquals(self.transaction_manager.sites_transactions_accessed_log["T1"], \
                          set([self.transaction_manager.sites[2]])) # Un-replicated variable
