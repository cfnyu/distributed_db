        ...
        if instruction.instruction_type == InstructionType.BEGIN:
            ...
            for site_identifier, site in self.sites.iteritems():
                if site.status == SiteStatus.UP:
                    for variable_id in site.data_manager.variables:
                        if site_identifier not in self.readonly_snapshots[trans_ident]:
                            self.readonly_snapshots[trans_ident][site_identifier] = []
                        self.readonly_snapshots[trans_ident][site_identifier].append(site.data_manager.variables[variable_id])
                ...
                if site.status == SiteStatus.UP:
                    if transaction.transaction_type == TransactionType.READ_ONLY:
                        if site_id in self.readonly_snapshots[transaction.identifier]:
                            for variable in self.readonly_snapshots[transaction.identifier][site_id]:
                                if variable.identifier == instruction.variable_identifier:
                                    print "%s: Read %s - value %s at site %s" % (instruction.transaction_identifier, instruction.variable_identifier, str(variable.value), str(site_id))
